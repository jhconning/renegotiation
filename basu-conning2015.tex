...
\]
which if \frame{$\delta=1/(1+r)$} implies that at an optimum c_{1}=c_{2}=\overline{c}.$ \ For the case of CRRA utility $u(c)=\frac{c^{1-\rho}%
}{1-\rho}$ \ we have $u^{\prime}(c)=c^{-\rho}$ and hence $c_{0}^{-\rho}%
=\beta\delta(1+r)\overline{c}^{-\rho}$ or
