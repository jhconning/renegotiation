...
\begin{equation}
  \Pi_{1}(C;C_1^0) \ge  \kappa  \label{PiGain}
\end{equation} 
Closed form solutions for $ C_1^{1}(C_1^0)$ and $C_1^{1m}(C_1^0)$ for the CRRA utility case are derived in the appendix. 
...
\label{eq:no_reneg}
\end{equation}
Here  we will abuse notation slightly for a moment to allow $C_1^1$ to represent the outcome in either the competitive or the monopoly subgame (i.e. in the latter case $C_1^{1}(C_{1}^{0})$ is  $C_1^{m1}(C_{1}^{0})$) replaces $C_1^1(C_{1}^{0})$. Notice that in our earlier statement of the competitive renegotiated contract  $C_1^1(C_{1}^{0})$ condition \ref{PiGain} was added to make sure the bank could earn enough from renegotiation to cover renegotiation costs and entice it to participate.  This no-renegotiation constraint now replaces that constraint to make sure bank gains from renegotiation fall just short of costs to make sure the bank cannot profit from renegotiation. Unless the parties in period 0 can take actions to increase renegotiation costs $\kappa$ (explored later in the paper) an effect of this constraint will be that equilibrium contracts now `tilt' more toward One-self's preferences as a strategy to reduce the remaining potential gains to trade to recontracting below renegotiation costs.     
 
We can now describe the design problem for period 0 contracts with sophisticated consumers who anticipate the renegotiation problem.  When the market for period 0 contracts is competitive the optimal (imperfect) commitment contract $C_0^{0} (\kappa)$ depends on available exogenous renegotiation costs $\kappa$ and solves:  
...
\end{equation}
where $C_1^{0}$ is the `best response' contract from the period 1 subgame as previously described. 
When renegotiation costs $\kappa$ are sufficiently large, that is larger than some threshold $\bar \kappa$ the renegotiation-proof constraint will not bind and the full-commitment competitive contract $C_0 ^{0}(\kappa)=C_0^F$ can be achieved.
 When  renegotiation costs $\kappa$ are less than this threshold only a second-best `imperfect commitment' contract can be attained, at the expense of Zero-self's utility, as described below.

If instead the period 0 market is monopolized then the optimal (imperfect) commitment contract $C_0^{0} (\kappa)$ solves:

\begin{displaymath}
C_0 ^{0}(\kappa)= \arg \max_{C_0} \Pi_{0}(C_{0};Y_{0})
\end{displaymath}
...
\subsection{When do Full Commitment contracts survive?}

What  is the minimum renegotiation cost $\bar \kappa$ the survival of full-commitment contracts by deterring renegotiation?
The question can be studied graphically and mathematically. Suppose that $F$ in Figure 2 is the continuation $C_1^F$ of a full commitment  competitive contract from period 0. The cost to the bank of continuing to honor the contract is 
$c_1^{F}+c_2^{F}=2c_1^F$ or distance $ON$ on the diagram (measured in period 1 units).  Under monopoly in period 1 and   $\kappa=0$ renegotiation would be to  $R$ which lower the bank's costs from $ON$ to $OQ$, for a gain in profits $QN$. The bank would renegotiatiate to the same point so long as the renegotiation cost $\kappa$ is smaller than $QN$. If on the other hand we had competition for period 1 contracts, with $\kappa=0$ One-self would be able to recontract to $P$.  
With competition and $\kappa$ positive but not large enough to sustain $F$, renegotiation would be to a point along the segment $RP$ (to satisfy  the FOC\ $u'(c_1^1)=\beta u'(c_2^1))$ that lowers the contract cost to the bank by just enough to entice it to participate and cover its renegotiation costs $\kappa$. In summary, to   dissuade renegotiation from a full-commitment contract under either competition or monopoly for period 1 contracts we need to find the coordinates at $R$ or $C_1^R}=(c_1^1(C_{1}^{F}),c_2^1(C_{1}^{F}))$ and  associated renegotiated continuation contract cost 
$c_1^R+c_2^R$ or length $ON$ in the figure. 

The renegotiation cost $\bar \kappa$ that just deters renegotiation must be large to exceed the gains to trade so we must have the bank earning non-positive profits:
\begin{equation}
...
\end{equation}
and one of these inequalities must be strict to be sure at least one of the parties will reject the renegotiation. In the above expressions we've used the fact that under full-commitment $c_1^F =c_2^F$ and that a renegotiated contract will have $c_2^R=\beta^\frac{1}{\rho}c_1^R$. We'll adopt the convention that in such threshold cases it's always the bank's constraint that will hold with strict equality so that we refer to this as the `no-renegotiation' constraint throughout the analysis. 

 From \ref{eq:noreg_kbar} we have
 $c_1^R=\frac{2c_1^F-\kappa}{1+\beta^\frac{1}{\rho}}$ which allows us to solve
\begin{equation}
 ...
 \right]\label{eq:kbar}
\end{equation}
Since we  already know (from \ref{eq:CF1}) that $c_1^F={y}/({1+2\beta^\frac{1}{\rho}})$ this value depends only on $y,\beta$ and $\rho$. This means that the survival of full-commitment under competition is independent
of autarky utility.

In the case of period 0 full-commitment contracts under monopoly, by identical reasoning  we are led to a cutoff $\bar \kappa^{M}$ given by formula \ref{eq:kbar} but with $c_1^{mF}$ (given by \ref{eq:c-mf}) replacing $c_1^F$. Monopoly full-commitment contracts depend on autarky utility\textendash the
higher $U_{0}^{A}$, the higher is $c_{1}^{mF}$. Since $c_1^F \ge c_1^{mF}$ we must have $\bar \kappa^m \le \bar \kappa$. We show in the appendix
...
\begin{prop}
Consider $\bar{\kappa}$ as defined in \ref{eq:kbar}.
