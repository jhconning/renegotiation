...
2 consumption to boost period 1 consumption instead. Like a Stackelberg-leader in a Cournot
...
To illustrate, at these parameters, Zero's preferred contract is $C_{0}^{F}=(150,75,75)$.
Whether the consumer borrows or saves (or pays down debts) in any given
period depends on how this consumption stream matches her autarky
consumption stream. If, for example,  the total income of 300 arrives evenly across periods as $Y_{0}=(100,100,100)$ then this consumption plan
would imply borrowing $c_{0}-y_{0}=50$ in period 0 to be repaid as installments of
25 in each of periods 1 and 2. If the stream had instead been $Y_{0}=(200,50,50)$
the consumer would be seen as saving 50 in period 0 to  raise consumption by 25 in each of periods 1 and 2.\footnote{These parameter values are chosen for expositional purposes. In particular
...
s.t. & \;U_{0}\left(C_{0}\right)\geq U_{0}^{A}(Y_0)\label{eq:PC}
\end{align}
where $U_{0}^{A}$ is the reservation utility the consumer would enjoy
if she did not accept the contract.

The first-order tangency conditions are  the same as competitive case given by  expressions \ref{eq:FOC_comp}. Under the maintained assumption that $\delta=1/(1+r)$
and $r=0$ these are:
\begin{equation}
u'\left(c_{0}\right)=\beta u'\left(c_{1}\right)=\beta u'\left(c_{2}\right)
\end{equation}
which for CRRA utility imply $c_{1}=c_{2}=\beta^{\frac{1}{\rho}}c_{0}$.
...
utility $U_{0}^{A}(Y_0)$. The present value of $C_{0}^{mF}$ rises and profits fall
with $U_{0}^{A}$. Since the monopolist retains the gains from trade and consumer iso-utility (or indifference) surfaces do not cross,
consumption in each period under monopoly will be strictly lower than under competition  at all $Y_0$ except in the extreme case where autarky utility is already optimal ($U_{0}^{A}=U_{0}^{F}$),
...
credible, how is it sustained, and at what cost? At issue is the fact that One-self always prefers higher period 1 consumption than Zero wants to build into a contract so in any period 1 subgame between  One-self and the (same or a different)\ bank there are tempting potential gains
to trade  from breaking earlier contract commitments to Zero.  The credibility of the full-commitment contract must rest rest on the threat of a sufficiently  costly punishment deterring the bank from engaging in such renegotiations. In this section we determine the minimum size of the  punishment required to deter the bank from breaking  the full-commitment contract. We also characterize the second-best `imperfect commitment' contracts that may be sustained if the available deterrent punishments fall short of this threshold.  In later sections we examine some of the ex-ante costly strategies that banks  might pursue to make their commitments more credible.   

One-self values period 1  consumption more highly relative to period 2 consumption  than Zero does and  will therefore generally like renegotiate the terms of the continuation contract $C_1^0$ inherited from period 0. Let us label the `best response' contract chosen in the subgame
of period 1 as $C_1^1(C_1^{0})$ but note that its terms and therefore also the division of the gains to trade will depend on the period 1 market for refinances is competitive or monopolized (or something in between).  The contract will be renegotiated if the gains to trade are sufficient to  cover the bank's renegotiation costs $\kappa$. In this subsection we focus on understanding  how contracts could be renegotiated in period 1 and their harmful consequences on consumer welfare and/or bank profits in period 0. In the following subsection we will then add renegotiation-proof constraints that avoid these costly renegotiations but at some efficiency cost.


Consider first the situation where the market for refinanced contracts is competitive (i.e. the period 0 bank cannot enforce an exclusive contract).  For the moment it does not matter
whether there was competition or monopoly in period 0.  
 Competition in period 1 will lead banks to offer to renegotiate the existing continuation contract $C_1^{0}$ for a new contract $C_1^1(C_1^{0})$, allowing the (new or existing) bank to just break even after covering any renegotiation costs:
\begin{equation*}
C_1^1(C_1^0)= \arg \max_{C} \ U(C)
\end{equation*}
\begin{equation}
  \Pi_{1}(C;C_1^0) \ge  \kappa  \label{PiGain}
\end{equation} 
or in more detail: 

\begin{equation}
(c_1^{1},c_2^{1})= \arg \max_{c_1,c_2}  \ u(c_1)+\beta u(c_2) 
\end{equation}
$$   c_1^0 +c_2^0 -(c_1 +c_2) \ge \kappa$
This can be solved for $C_1^1(C_1^0)$ in the CRRA case:  
\begin{equation}
(c_1^1, c_2^1) = (1,\beta^\frac{1}{\rho})\cdot \frac{c_1^0 +c_2^0-\kappa}{1+\beta^\frac{1}{\rho}}
\end{equation}

If instead the bank finds itself in a monopoly position in period 1 then it will renegotiate existing contract $C_1^0$ to a new contract $C_1^{m1}(C_1^{0})$ to increase profits by offering the smallest possible enticement for One to accept the renegotiated contract. The bank must be able to cover renegotiation costs if a contract is to be offered at all. 

\begin{equation*}
C_1^{1m}(C_1)= \arg \max_{C} \ \Pi_{1}(C;C_1^0) 
\end{equation*}
\begin{equation}
  U(C) \geq U(C_1^0)
\end{equation}
\begin{equation}
  \Pi_{1}(C;C_1^0) \ge  \kappa  \label{PiGain}
\end{equation} 
Closed form solutions for $ C_1^{1m}(C_1^0)$ is derived in the appendix for the CRRA utility case. 

The fraught nature of this potential renegotiation problem is  depicted in Figure \ref{fig:c1c2}, for a case where renegotiation costs are set to $\kappa = 0$.  Assume -- just for the sake of argument  now  -- that the consumer had (naively as it will turn out) accepted
the competitive full commitment contract $C_{0}^{F}$ in period zero, indicated as point $F$ in the figure. 
This contract satisfies Zero-self's optimality condition $u'(c_{1}^{F})=u'(c_{2}^{F})$
as seen by the fact that Zero-self's indifference curve is tangent
to the bank's zero profit line. For One's preferences  however this consumption bundle involves too little
period 1 consumption since $u'(c_{1}^{F})\geq\beta u'(c_{2}^{F})$ and as can be seen from the fact that at $F$  One-self's indifference curve is steeper than the budget line. With zero renegotiation costs ($\kappa=0$) there are gains-to-trade that can be shared from recontracting from $F$ to any
new tangency point (along the $c_{2}=\beta^{\frac{1}{\rho}}c_{1}$ ray where One's FOCs are met) between point $R$  which is the contract $C_1^{m1}(C_1^F)$ least favorable to One (chosen if  the bank could act as monopolist in period 1) and below point $P$ which is  the contract $C_1^{1}(C_1^F)$ most favorable to One (chosen with competitive refinancing). 
Zero is clearly worse off with  any such refinancing away from  its preferred optimal contract $F$. 
...
A similar problem arises  in period 1 when the bank has monopoly power in period 0. We can re-purpose Figure 2 by re-interpreting $F$ to correspond to the last two periods of consumption from contract $C_{0}^{mF}$.  Here too there are gains-to-trade from recontracting from $F$ to any
new tangency point (along the $c_{2}=\beta^{\frac{1}{\rho}}c_{1}$ ray) between a new point $R$  and below a new point $P$. Either situation would push Zero's intertemporal utility below what had been expected from $F$. 
\subsection{Renegotiation-proof contracts}

Being  forward looking and sophisticated, Zero-self  anticipates these possibilities of opportunistic recontracting
and will seek to protect herself by only agreeing to contracts that satisfy a period 1 no-renegotiation constraint.   \begin{equation}
\Pi_{1}(C_1^{0};C_1^{1}(C_1^{0})) \le \kappa 
\label{eq:no_reneg}
\end{equation}
In the case of a period 1 monopoly subgame the constraint which change slightly with $C_1^{m1}(C_{1}^{0})$ replacing $C_1^1(C_{1}^{0})$ in the expression above. To be clear, when we solved for   $C_1^1(C_{1}^{0})$ in the last section we included condition \ref{PiGain} (which looks similar to \ref{eq:no_reneg} but with the inequality reversed)\ to entice the bank to participate by making sure that the bank earned enough from renegotiation to cover renegotiation costs. We imposed this to identify contracts the bank would \emph{not reject} to renegotiate.
 
In this section our focus is instead on searching amongst the best contracts the bank would \textit{reject}  renegotiating.  Thus the no-renegotiation constraint \ref{eq:no_reneg} is to makes sure the bank \textit{cannot} profit from renegotiation. If this constraint binds a full-commitment contract cannot be credibly implemented and the equilibrium contracts will instead now `tilt' more toward One-self's preferences as a strategy to reduce the potential gains to trade in period 1 that might tempt renegotiation.     
 
The design problem for period 0 contracts with sophisticated consumers who anticipate the renegotiation problem can now be described.  When the market for period 0 contracts is competitive the optimal (imperfect) commitment contract $C_0^{0} (\kappa)$ depends on available exogenous renegotiation costs $\kappa$ and solves:  

\begin{equation}
C_0 ^{0}(\kappa)= \arg \max_{C_0} U_{0}(C_{0})
\end{equation}
...
\begin{equation}
\Pi_1(C_1^{1}(C_1^{0}),C_1^{0})\le\kappa
\end{equation}
where $C_1^{0}$ is the `best response' or renegotiated contract from the period 1 subgame as previously described. 
When renegotiation costs $\kappa$ are sufficiently large -- larger than some threshold $\bar \kappa$ described below -- the renegotiation-proof constraint will not bind and the full-commitment competitive contract $C_0 ^{0}(\kappa)=C_0^F$ can be implemented as a subgame-perfect Nash equilibrium\ (SPNE).
 When  renegotiation costs $\kappa$ are less than this threshold only a second-best `imperfect commitment' contract can be implemented as a SPNE, at the expense of Zero-self's utility, as described below.

If instead the period 0 market had been monopolized then the optimal (imperfect) commitment contract $C_0^{m0} (\kappa)$ solves:

\begin{displaymath}
C_0 ^{m0}(\kappa)= \arg \max_{C_0} \Pi_{0}(C_{0};Y_{0})
\end{displaymath}
...
\Pi_1(C_1^{1}(C_1^{0}),C_1^{0})\le\kappa
\end{equation}
Here too, when renegotiation costs $\kappa$ are sufficiently large the the monopoly full-commitment competitive contract $C_0 ^{0}(\kappa)=C_0^{mF}$ can be achieved.
 When  renegotiation costs $\kappa$ are less than only a second-best `imperfect commitment' contract can be attained, at the expense of period 0 bank profits.



\subsection{At what cost are full-commitment contracts kept credible?}

What  minimum renegotiation cost $\bar \kappa$ is sufficient to deter the renegotiation of  full-commitment contracts?
We study the question graphically and mathematically. Suppose that $F$ in Figure 2 is the continuation $C_1^F$ of a full commitment  competitive contract offered in period 0. The cost to the bank of continuing to honor the contract is 
$c_1^{F}+c_2^{F}=2c_1^F$ or the distance $ON$ on the diagram (measured in period 1 units).  Under monopoly in period 1 and   $\kappa=0$ renegotiation to  $R$ would lower the bank's costs from $ON$ to $OQ$, for a profit gain $QN$. In fact the bank will find it profitable to renegotiate to point $R$ at any renegotiation cost $\kappa$ smaller than $QN$. If instead the   market for period 1 refinance contracts were competitive and $\kappa=0$, One-self would capture the new gains to trade by recontracting to $P$.  
With competition and $\kappa$ positive but not large enough to sustain  $F$, renegotiation would be to a point along the segment $RP$ (where the FOC\ $u'(c_1^1)=\beta u'(c_2^1)$ is satisfied) that just entices the bank to refinance and cover its renegotiation costs $\kappa$.  In summary, under either competition or monopoly for period 1 contracts renegotiation away from $F$ is deterred whenever renegotiation cost $\kappa$  exceeds the gains to trade $QN$ from renegotiation. 

To calculate the gross gains to trade $QN$ we need to first find the coordinates at $R$ or $C_1^R}=(c_1^1(C_{1}^{F}),c_2^1(C_{1}^{F}))$ and  associated renegotiated continuation contract cost 
$c_1^R+c_2^R=(1+\beta^\frac{1}{\rho})c_1^R $ or length $OQ$ in the figure and subtract this quantity from continuation contract cost at $F$ or the distance $ON$ calculated above. 
The renegotiation cost $\bar \kappa$ that just prevents renegotiation must therefore satisfy the condition that the bank earns non-positive profit from renegotiation:
\begin{equation}
  2c_{1}^{F}}-(1+\beta^\frac{1}{\rho})c_1^R \le \bar \kappa \label{eq:noreg_kbar}
\end{equation} 
and the One-self consumer must have non-positive welfare gain from renegotiation   
\begin{equation}
(1+\beta)u(c_1^F)\ge (1+\beta^\frac{1}{\rho})u(c_1^R) 
\end{equation}
and one of these inequalities must be strictly met to make sure that at least one of the parties will reject renegotiation. In what follows, and without loss of generality,  we'll adopt the convention that it's always the bank's constraint that will hold with strict equality so we refer to  \ref{eq:noreg_kbar} as the `no-renegotiation' constraint throughout the analysis. 

 From this constraint we have
 $c_1^R=\frac{2c_1^F-\kappa}{1+\beta^\frac{1}{\rho}}$ which allows us to solve for the minimum deterrent 
\begin{equation}
 ...
 \right]\label{eq:kbar}
\end{equation}
The value of $c_1^F$ in the expression will differ depending on whether the period 0 market for full-commitment contracts is competitive or monopolized.  Under competition we know from \ref{eq:CF1} that $c_1^F={y}/({1+2\beta^\frac{1}{\rho}})$ so $\bar \kappa$ depends only on $y,\beta$ and $\rho$. This means also that the survival of full-commitment under competition is independent
of autarky utility.

In the case of period 0 full-commitment contracts under monopoly we are led to a cutoff $\bar \kappa^{M}$ given by formula \ref{eq:kbar} but with $c_1^{mF}$ (given by \ref{eq:c-mf}) replacing $c_1^F$. Monopoly full-commitment contracts now depend on autarky utility \textendash  the
higher $U_{0}^{A}(Y_0)$, the higher is $c_{1}^{mF}$. Since $c_1^F \ge c_1^{mF}$ for all $Y_0$ it will always be the case that  $\bar \kappa^m \le \bar \kappa$. We show in the appendix
...
Together, this indicates that, if renegotiation costs lie above cutoff $\bar \kappa$, full commitment contracts will be credible; and if renegotiation
costs lie below this cutoff, the survival of full-commitment will depend
on market structure and autarky utility. 


\begin{prop}
Consider $\bar{\kappa}$ as defined in \ref{eq:kbar}.\footnote{All proofs are in the appendix.}
If $\kappa\geq\bar{\kappa}$, then full-commitment contracts survive
under both monopoly and competition. If $\kappa<\bar{\kappa}$, then:

(a) The competitive full commitment contract $C_{0}^{F}$ will be
renegotiated at any $U_{0}^{A}$.

(b) There is some $U^{m}<U_{0}^{F}$ such that the monopolist full
commitment contract $C_{0}^{mF}$ will be renegotiated if and only
if $U_{0}^{A}>U^{m}$.
 
\end{prop}
Note that the monopolist's ability to commit at lower (threatened renegotiation cost) comes from the fact that its commitment contract is less susceptible to renegotiation. That is because, having, at the outset, extracted surplus by offering the consumer a contract with the lowest possible consumption, there is relatively less for the firm to capture via renegotiating in period 1 (from the derivation of \ref{eq:kbar} it is clear that the gains to renegotiation are proportional to the cost of the continuation full-commitment contract and we know $c_1^F \ge c_1^{mF}$   ).
A further implication is that, under monopoly, consumers with already relatively
...
We turn now to the more interesting case where
 full-commitment contracts cannot be sustained and the feasible contract set will be constrained by the fact that renegotiation-proof constraint \ref{eq:no_reneg} binds. We call
...
\subsection{The $\kappa=0$ case}

The special case of perfect competition with $\kappa=0$, illustrated
in Figure 1b. This differs from Figure 1a in one crucial respect:
any amount not consumed in period 0 is no longer divided between periods 1 and
2 equally as in the full-commitment case but according to One-self's wishes (since, with $\kappa=0,$
...
The contract design problem is like a Stackelberg game with Zero-self moving first
to choose a contract in anticipation of One-self's best response in the subgame entered following Zero's choice of contract $C_{0}$.
In this subgame One self can now either accept the remaining consumption plan $C_{1}^0$
...
be rewritten 
\begin{equation}
c_{1}+c_{2}\leq y-c_{0}^{0}=s
\end{equation}
from which it becomes clear that One-self needs to react only to Zero's
period zero consumption $c_{0}$ or, same thing, net saving of resources in period 0 as this establishes the remaining
...
One-self would like to re-arrange this plan to satisfy her preferences
...
\subsection{Monopoly with $\kappa>0$}
...
C_{0}^{mF} & =\left(1,\beta^{\frac{1}{\rho}},\beta^{\frac{1}{\rho}}\right)
  \cdot\left(
     \frac{U_{0}^{A}\left(1-\rho\right)}{1+2\beta^{\frac{1}{\rho}}}
     \right)^{\frac{1}{1-\rho}}\label{eq:c-mf}\\
...
Given an existing continuation contract  $C_{1}^0$, if there is competition for renegotiated contracts then zero profit condition in the period 1 subgame will bind (the bank must cover renegotiation costs)
\begin{displaymath}
c_1+c_2+\kappa=c_1^0 +c_2^0
\end{displaymath} 
and the competitively renegotiated contract will be:
\begin{equation}
C_{1}^{1}\left(C_{1}^{0}\right)=\left(1,\beta^{\frac{1}{\rho}}\right)\cdot\left(\frac{c_1^0 +c_2^0-\kappa}{1+\beta^{\frac{1}{\rho}}}\right)
\end{equation}
 

Given an existing contract $C_{1}^0$, a monopolist bank will renegotiate
in period 1 and  offer the following new contract: 
\begin{equation}
C_{1}^{m1}\left(C_1^0\right)=\left(1,\beta^{\frac{1}{\rho}}\right)
\cdot\left(\frac{(c_{1}^0)^{1-\rho}
+\beta (c_{2}^0)^{1-\rho}}{1+\beta^{\frac{1}{\rho}}}\right)^{\frac{1}{1-\rho}}
\end{equation}\label{eq:c-r}

In both cases we are assuming that $\kappa$ is not large enought to eliminate the gains to trade. The corresponding profit gains from renegotiation in the monopoly case are: 
\begin{equation}
\Pi_{1}\left(C_{1}^{m1}\left(C_{1}^0\right);
C_{1}^0\right)=\left(c_{1}^{0}+c_{2}^{0  }\right)
-\left((c_{1}^0)^{1-\rho}
+\beta (c_{2}^0)^{1-\rho}\right)^{\frac{1}{1-\rho}}
\left(1+\beta^{\frac{1}{\rho}}\right)^{\frac{-\rho}{1-\rho}}\label{eq:pi-r}
\end{equation}
...
\begin{equation}
\Pi_{1}\left(C_{1}^{m1}\left(C_{1}\right);C_{1}\right)=\left(s\right)\left(1-\left(\alpha^{1-\rho}+\beta\left(1-\alpha\right)^{1-\rho}\right)^{\frac{1}{1-\rho}}\\ \left(1+\beta^{\frac{1}{\rho}}\right)^{\frac{-\rho}{1-\rho}}\right)
\label{eq:renegotiation-profits}
\end{equation}
...
\end{equation}
Substituting for $s=y-c_0^F$ from the competitive full-commitment contract,
...
is sufficiently large. Since $s^{mF}$ is exponentially (and therefore monotonically) increasing
