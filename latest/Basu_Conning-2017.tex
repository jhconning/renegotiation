...
and/or debt management plans?\  When  instead do might they opportunistically
exploit and pander to their consumers' biases? How much is  financial trade distorted by consumer fears that this could happen? We study a consumer
...
take on more debt. In the absence of effective consumer protection regulation, investor-owned banks may find it costly to credibly
commit to not engage in such opportunistic exploitation leading to
...
forms can lower the cost of making credible (or renegotiation-proof) commitment
contracts as socially-minded owners and/or explicit legal limits on profit distribution  weaken incentives to opportunistic renegotiation. This leads to a theory of
commercial non-profits and endogenous client protection similar to
\citet{hansmann_ownership_1996} but built on new behavioral micro-foundations.
...
and to frame debates over consumer protection measures against
...
denoted $U_{0}^{A}\left(Y_{0}\right)$. Autarky utility might be from  consuming  their autarky income stream   if the consumer has no  consumption smoothing strategies or it might be the utility achieved after some smoothing via `own-savings' or
informal borrowing, if available. Section \ref{sec-FCC} defines Zero-self's benchmark efficient consumption smoothing stream $C_0^F$ and associated utility level $U_{0}^{F}$ which is what could be achieved if she had perfect access to borrowing and saving at competitive interest rates,
plus the commitment services to make sure the contract is not renegotiated. In general, there are many reasons why under autarky consumption plans should fall short of this perfect consumption smoothing benchmark. For example, if her income is back-heavy, borrowing
constraints might mean that she must consume her income as it arrives. If
her income is front-heavy the
consumer may be able to construct a somewhat better smoothed autarky
stream but there may be technological restrictions to saving
that place the own return to savings well below the market rate, with insecurity of storing cash at home being one obvious
explanation. More pivotal to our analysis, however, is the fact that
...
path \cite[see][and Section 3 below]{basu_commitment_2014}. While
...
We first establish benchmark efficient  full smoothing commitment contracts under
 competition and monopoly, respectively. We define such contracts to be achieved when a bank and Zero-self  enter into an multi-period contract that sustains efficient consumption smoothing by binding the consumer's
latter period self(ves) to not renegotiate the contract terms with the same bank or
other banks.  Sophisticated present-biased  consumer will be willing to pay for the `commitment services'  that  a bank might offer. It should be clear however that a bank's willingness to remain committed to help the consumer `stick-to' the contract or --same thing -- to not try to opportunistically profit by renegotiating the terms of the earlier agreed multi-period contract must itself rest upon the bank facing the prospect of a renegotiation penalty ($\kappa$) that is  sufficiently credible and high to deter any such renegotiation.  When the contracting environment is such that the highest penalty $\kappa$ that the bank can credibly commit to face is smaller than this level necessary to sustain efficient perfect smoothing then efficient smoothing commitment
contracts will no longer be sustained. Efficient smoothing is then replaced by self-enforcing second-best
renegotiation-proof contracts. We later delve into what in practice might determine $\kappa$ and how it might be endogenously
determined via choices of bank ownership and governance forms and
also shaped by the nature of the market structure.
...
\end{equation}
Solving for an interior solution using first-order conditions and the binding condition \ref{eq:ugain}, the CRRA case yields a closed form solution (\ref{eq:m-r}). In figure ?? contract $C_{1}^{0}$ at \textbf{P} would be renegotiated to point \textbf{$R^{m}$} where the ray $c_{2}^{1}=\beta^{\frac{1}{\rho}}c_{1}^{1}$ intersects One-self's participation constraint  \ref{eq:PiGain}.
...
\subsection{Renegotiation-proof contracts with $(0\leq\kappa<\bar{\kappa})$ }
...
\item [{(ii)}] The relationship between $c_{0}^{P}$ and $c_{0}^{F}$ is
ambiguous. There is some $\hat{\rho}$ such that: if $\rho\leq\hat{\rho}$,
then $c_{0}^{P}>c_{0}^{F}$; if $\rho>\hat{\rho}$, then there are
parameter values under which $c_{0}^{P}<c_{0}^{F}$. 
\item [{(iii)}] There is some $\bar{U}<$$U^{F}$ such that, if $U_{0}^{A}>\bar{U}$,
...
\item [{(ii)}] $c_{0}^{mP}>c_{0}^{mF}$. 
\item [{(iii)}] If $U_{0}^{A}>\bar{U}$, the bank would earn negative profits
...
\underset{C_{0}}{\mbox{\ensuremath{max}}} & U_{0}\left(C_{0}\right)\nonumber \\
 & \text{s.t.  }\Pi_{0}\left(C_{0};Y_{0}\right)+\Pi_{1}\left(C_{1}^{m1}\left(C_{1}\right);C_{1}\right)\geq\kappa
\end{align}
...
We consider now the case of a firm that in a pre-contract stage has the possibility of changing its ownership structure and/or legal form for example by choosing  to incorporate as a legal non-profit or, more broadly, by becoming a  `hybrid' ownership firm, for example as a for-profit non-bank financial intermediary  where social investors (including for example non-profit charitable foundations) exercise considerable ownership and managerial control. Changes to the firm's legal status and ownership and governance structure can affect the firm's expected behavior
and hence the credibility of its contractual promises via two broad mechanisms. Firstly, by directly reshaping the objectives and motivations of the firm's principals. A firm that is dominated by social investors or customer shareholders is more likely to hire managers and employees that place customer welfare ahead of firm profits compared to a firm dominated by for-profit outside investors.  Secondly, ownership and legal form can shape the firm's choices and the quality of its promises by relaxing or tightening the constraints faced by the firm's principals without changing their motivations.  These effects may work separately or in combination. 

 To illustrate this in a simple yet still rich manner let us index a firms' ownership structure and degree of `profit orientation' via a simple parameter
$\alpha\epsilon(0,1]$.
  The second of the mechanisms can be expressed by assuming that $\alpha$ indicates the fraction of raw profits that can be distributed to  managers and shareholders:\  if a firm produces profits $\Pi$, its principals are constrained by law or governance to capture only $\alpha\Pi$. This follows \citet{glaeser_not-for-profit_2001}
who argue that this captures the idea that though the principals of a non-profit may be technically legally barred from tying compensation to cash profits, in practice they can often capture a fraction of profits in costly and imperfect ways via the consumption
of perquisites  or `dividends in kind' (e.g. a lavish expense account).  An alternative interpretation is simply that an increase in control by social investors is more likely to lead to the adoption of credible voluntary caps on profits and their distribution.  
  
We could also allow $\alpha$ to directly alter the  firm management's objectives. For example, it could be argued that the non-pecuniary renegotiation cost the principals' of the firm incur when breaking
contractual  promises to its customers will be higher for lower $\alpha$ firms as social investors feel more guilt or shame or fear the loss of reputation compared to narrowly profit-oriented investors.
If we  label the cost of renegotiation as $\eta(\alpha)$ -- replacing our earlier $\kappa$ -- this idea can be  captured by allowing
$\eta$ to fall weakly in $\alpha$.

These assumptions lead to a modified no-renegotiation constraint
(\ref{eq:no_reneg_np}) which   states that the value
of profits that can be captured from renegotiating the contract must not exceed
renegotiation costs. 
\begin{equation}
\alpha\Pi_{1}(C_{1}^{m1}(C_{1});C_{1})\leq\eta(\alpha)\label{eq:no_reneg_np}
\end{equation}
If we now define $\kappa(\alpha)\equiv\frac{\eta(\alpha)}{\alpha}$,
this no-renegotiation constraint can be rewritten as

\begin{equation}
\Pi_{1}(C_{1}^{m1}(C_{1});C_{1})\leq\kappa(\alpha) \label{eq:no-kalpha}
\end{equation}
This is like the earlier no-renegotiation constraint except that $\kappa$ is now a function
of $\alpha$. The earlier analyzed renegotiation problems are now just the  special case with $\alpha=1$.

Before proceeding, it should be noted that profit-oriented principals never have an incentive
...
To isolate key mechanisms we will start by analyzing cases where the firms' principals care only about their pecuniary earnings and the choice of non-profit or hybrid status works entirely via the mechanism that a change in legal/ownership status changes the   share of raw profits that can be captured by those principals rather than through any direct change in the firms' objectives. That is to say we set $\eta(\alpha)=\eta$ to be a constant so $\kappa(\alpha)=\eta/\alpha$).
We derive conditions under which such firms might voluntarily switch from full for-profit (i.e. $\alpha=1$) status to non-profit or hybrid ownership/governance forms with $\alpha<1$. This is the scenario one might most readily associate with the case for a  pure commercial non-profit of \cite{hansmann_ownership_1996}. We later expand the analysis to allow renegotiation costs $\eta$ to vary directly (and inversely) with $\alpha$ to capture the that changes in ownership might also bring in motivated-agents to management or employment.  

\subsection{Monopoly}

The firm now first establishes its type $\alpha$
in a pre-contract phase via the adoption of legal non-profit status and/or by choosing credible
...
\begin{align}
 ...
 & \alpha \cdot \Pi_{1}\left(C_{1}^{m1}\left(C_{1}\right);C_{1}\right)\leq \eta(\alpha)  \tag{\ref{eq:no_reneg_np}}
\end{align}
Where $\kappa(\alpha)=\frac{\eta}{\alpha}$.  From  Proposition \ref{prop1}, the pure for-profit firm defined by $\alpha=1$ will be unable to offer full-smoothing commitment if constant $\kappa(1) =\eta<\bar \kappa$.  The pure for-profit firm captures 100 percent of any `raw profits' but these are not as large as those that could be generated by a firm facing a credibly larger renegotiation cost which can offer fuller consumption smoothing. 

If we lower $\alpha$ below 1 the no-renegotiation constraint \ref{eq:no-kalpha} will be relaxed and  previously infeasible (non renegotiation-proof) contracts with better consumption smoothing now become feasible making room for raw profits increase.  Let's refer to the new captured-profit maximizing contract when  \alpha<1$ as $C_{0}^{mP}(\alpha)$.
 With a relaxed renegotiation-proof constraint
$\Pi_{0}(C_{0}^{m}(\alpha);Y)\geq\Pi_{0}(C_{0}^{mP}(1);Y)$ where C_{0}^{mP}(\alph11)$ is the optimal renegotiation-proof contract in the earlier analyzed pure for-profit case.  It will only be in the bank principals' interest to adopt the hybrid profit status associated with $\alpha$ if $\alpha\Pi_{0}(C_{0}^{mP}(\alpha);Y)\geq\Pi_{0}(C_{0}^{mP}(1);Y)$. 


Proposition \ref{prop4} describes conditions under which a firm will prefer to operate
as a hybrid. Since the pure for-profit firm's no-renegotiation constraint
binded, the hybrid can offer greater commitment and thereby extract
greater surplus or `raw profits' from the consumer through the contract signed in period
0. But does the rise in extracted surplus outweigh the
fact that all profits are now discounted by $\alpha$? It's the choice between\ a smaller share of a larger pie or all of a smaller pie.
To the extent that the loosening
of the no-renegotiation constraint happens through the right-hand
side of (i.e. via term $\eta\left(\alpha\right)$, which represents the
...
unchanged (as in the case of a commercial-non profit described in the proposition with $\eta$ constant), the firm is forced to address the
...
sufficient additional consumption smoothing benefits to cover costs. For
intermediate levels of autarky utility, a monopoly hybrid can credibly promise to not renegotiate contracts that a pure for-profit would renegotiate, so it offers contracts that earn positive profits
...
relative to the profits that a pure for-profit would have made, so the
firm prefers to operate as a hybrid nonprofit. As an example, consider an
...
fact that its principals give up a significant share of its profits
