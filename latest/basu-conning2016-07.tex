...
\end{displaymath}
\begin{displaymath}
\Pi_0(C_0,Y_0)=\sum\limits _{i=t}^{2} \frac{\left(y_{t}-c_{t}\right)}{\left(1+r}\right)^i} \geq 0
\end{displaymath}
...
\end{displaymath}
 These equations plus the binding budget constraint allow us to solve for $C^0$. Conceptually the equilibrium contract is given by the  tangency where the consumer has reached the highest feasible iso-utility surface  touching the hyper-plane defined by the budget. While a 3D visualization is cumbersome, insights into the problem can be had from the 2D contour plot diagrams below.         
...
\end{equation}
\begin{equation}
c_2=[\delta(1+r)]^\frac{1}{\rho}c_{1} \label{eq:FOCc1c2}
\end{equation} 
Combining this last equation and the budget constraint also gives:
\begin{equation}
c_1=\frac{\sum y -c_0}{1+\delta (1+r)} \label{eq:budgetplus}
\end{equation}

 If  we further limit assume    \(\delta=\frac{1}{1+r}\) then the above equations  are simply  \(c_1=\beta^\frac{1}{\rho}c_{0}_\), \(c_2=c_{1}_\) and $c_1 = (\sum y-c_0)/2 $ . In words: Zero self wants to bias or tilt consumption toward period zero consumption but then allocate consumption evenly across the remaining two periods. From the above a closed form solution for \(C_{0}\) is easily found: 
...
This solution be seen in the two-panel diagram of Figure \ref{fig:twoquad},  drawn for a consumer with $\beta = 0.5$ and $\rho=1$ and income with a present value of \(y=300\) spread over the three periods (we are assuming \(r=0\)).\footnote{These parameter values are chosen for expositional simplicity. In particular  $\rho=1$ will imply that period zero consumption will  be the same under full commitment as in the case where self-control is not possible, This simplifies certain graphical interpretation of the self-control problem but the analysis readily extends to other values.} 
The right hand panel is drawn in $c_0-c_1$ contract space.  Points along the ray OF satisfy first-order equation \ref{eq:FOCc0c1} while points along the ray MF satisfy  the second tangency equation \ref{eq:FOCc1c2}  and  the budget line (i.e. they satisfy equation  \ref{eq:budgetplus}).   The intersection at F identifies the coordinates $(c_0^F,c_1^F)$ of the full-commitment contract of equation \ref{eq:CF}. The left hand panel depicts the situation in $c_1-c_2$ space of the sliced plane  at $c_0=c_0^F$). Details of this last diagram can be seen (now rotated 90 degrees)  in Figure \ref{fig:c1c2}.  Points along ray OF satisfy the Euler equation \ref{eq:FOCc1c2}, while points along the line passing through DF satisfy the period 1 budget $c_1+c_2=\sum y - c_0^$ where $c_0=c_o^F$ in this case. Point F at the intersection of these lines identifies the coordinates $(c_1^F,c_2^F).$
...
Let us turn now analyze what will happen when the the self-control problem  cannot be solved and  One self cannot be stopped from recontracting.  One-self will want to indulge her new present-bias and hence `over-consume' in period 1  relative to what her Zero self would have preferred. We can think of this as One self      `raiding savings'  or `piling on new debt' in period 1 in ways that reduce  the resources left for period 2 consumption. 
Figure \ref{fig:c1c2} makes clear why this will happen.  For illustration purposes assume that the consumer had (naively as it will turn out) accepted the contract \(C_0^F\). At the start of period 1 this contract satisfies Zero self's optimality condition $u'(c_1^F)=u'(c_2^F)$ but from the standpoint of One self's preferences it involves too little period 1 consumption as u'(c_1^F)\geq \beta u'(c_2^F)$. This is evident from the fact that at point F, Zero self's indifference curve is tangent to the budget line but One self's indifference curve is steeper than the budget line. One self would  gain by recontracting to any feasible tangency point along the $c_2=\beta^\frac{1}{\rho}c_1$ segment above where her indifference curve passing through point F cuts the line and and   below the budget line.\footnote{ When $\rho \neq 1$ point P will not lie exactly on the budget line defined by $c_0+c_1=\sum y - c_o^F$ as $c_o^F \neq c_o^P$ but a similar interpretation still applies. } Any of these points lowers Zero self's inter-temporal utility. A sophisticated    Zero self consumer who  anticipates this problem can do not better  than accept the contract at P (as One self would renegotiate any other contract that satisfies the budget constraint to P anyways).  This contract will involve less saving/more debt in period 1 and lower overall utility compared to Zero self's  preferred contract F.

A sophisticated hyperbolic discounter who fully anticipates her own future
change of preferences finds herself locked into a strategic game with her latter self.\footnote{A naive hyperbolic discounter, on the other hand, may believe she
can achieve optimal utility though this will not be realized. See below.} This is like a Stackelberg game with Zero self moving first, choosing a contract in anticipation of One self's best response. }   
To see how this game will be played, consider One self's best response in the subgame defined by Zero self's choice of contract \(C^{0}=(c_0^0, c_1^0,c_2^0)  \). \ One self can either accept the remaining consumption plan  $(c_1^0,c_2^0)$ or exchange it for a new contract to solve
\begin{displaymath}
