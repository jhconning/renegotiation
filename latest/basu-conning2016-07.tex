...
\date{July 12, 2016 (preliminary draft)}
...
\subsection{Self-control with costless full-commitment contracts  } 
...
consumption stream using what financial contracting savings and/or borrowing strategies are available.  A consumer with time-inconsistent
preferences generally cannot trust her later selves to not disrupt her preferred long-term 
consumption plans.  In this simple three period setting Zero self's particular concern is that her One self will want to divert to period 1 consumption resources that Zero self would prefer to earmark for period 2 consumption. 
Zero self finds herself therefore in a  strategic contracting game with  her latter period self. A bank may be able to profitably act as a strategic partner to Zero by offering
contracts
with commitment services
that limit or otherwise control the customer's later self(ves) ability to recontract.  

In this section we contrast two initial polar opposite contracting situations under  conditions of competition and monopoly: (1)\ the full-commitment or full self-control case where we simply assume a bank can offer the Zero
consumer
a
multi-period contract that \textit{costlessly} binds her latter self(ves) to not renegotiate its terms with the bank or others; and (2) the no self-control case where the bank cannot offer a contract that prevents the consumer's latter self from recontracting.\footnote{Or can do so only in the narrow sense of offering a renegotiation-proof contract that the latter self would recontract to anyways.}   The consumption smoothing that can be achieved in this latter case will in general fall short of  what can achieved in the former, hence the demand for commitment services in the market.

This section therefore studies cases where self-control is either not possible (or prohibitively expensive)\ and cases where full self-control can be costlessly achived.  Once these benchmark cases are established we will then turn in the following section to the more interesting and realistic cases of where self-control
can be achieved only via costly self-enforcing renegotiation-proof\ contracts that provide incentives to both the customer and the bank to keep their pledges to not renegotiate contracts. 
\subsubsection{Competitive full-commitment contracts}
\label{sec:own}
...
Suppose also that the consumer faced no self control problem: Zero self chooses a contract that costlessly commits her period-1 and period-2 selves to follow the consumption plan laid out by the contract. This is  solved as a standard utility maximization problem subject to an   inter-temporal budget constraint
...
The contracts of the last section rested on the assumption that the customer's One self could be kept from changing the terms of the contract  either by refinancing/renegotiating with the same bank or with a different bank.  The latter could be prevented if the bank contract in period 0 could be made exclusive, which is to say that it could include enforceable clauses that effectively prevent the customer from contracting with a second bank. Whether exclusive contracts can be made enforceable will depend on details of the legal and institutional environment. For the moment let us maintain that exclusive contracts can be written and focus on what will be required to keep the bank's commitment to not renegotiate the contract in period 1 credible.  This is an important question since as can be easily demonstrated the customer's change of preferences in period 1 will mean that there will always be gains to trade to be split between the bank and/or period-1 self by breaking an earlier commitment, unless explicit or implicit costs have been built into the contract.     

To see the problem most clearly, consider now the situation where  self-control cannot be simply assumed. When period 1 arrives One-self will want to indulge her present-bias and  `over-consume' in period 1  relative Zero's preferences. The nature of this problem can be seen in Figure \ref{fig:c1c2} and (same thing) the left panel of Figure \ref{fig:twoquad}. Let us assume for the sake of argument that the consumer had (naively as it will turn out) accepted the contract \(C_0^F\) in period zero. At the start of period 1 this contract satisfies Zero self's optimality condition $u'(c_1^F)=u'(c_2^F)$ but from the standpoint of One's preferences it involves too little period 1 consumption since $u'(c_1^{0F})\geq \beta u'(c_2^{0F})$. At point F, Zero self's indifference curve is tangent to the budget line but One self's indifference curve is steeper than the budget line. One self can   clearly gain by recontracting to any feasible tangency point along the $c_2=\beta^\frac{1}{\rho}c_1$ ray, above where that indifference curve cuts the line and and   below the budget line and can entice the bank by offering to share some of the surplus.\footnote{ Figure \ref{fig:c1c2} assumes $\rho=1$ which will imply that in equilibrium $c_0^{mF}=c_0^{mP}$.  When $\rho \neq 1$ point P will not lie exactly on the budget line defined by $c_0+c_1=\sum y - c_o^F$ as $c_o^F \neq c_o^P$ but the analysis is readily adapted.}  From Zero's point of view the bank would be pandering to One self's desire to `raid savings' or `roll over or piling on new debt' relative to Zero's plans. 

A sophisticated    Zero self consumer anticipates this  recontracting and how it lowers their inter-temporal welfare\footnote{A naive hyperbolic discounter, on the other hand, may believe she
can achieve optimal utility though this will not be realized. See below.}. From the diagram it should be clear that Zero self can do no better  to limit the damage than to accept the contract at P (since any other feasible preferred contract would be recontracted to P anyways).  This subgame-perfect `renegotiation-proof' contract will involve less net saving (or same thing, more net debt) in period 1 than Zero self would like (as can be seen by $u'(c_1^{P})<  u'(c_2^{P})$) and lower Zero self welfare compared to the full-commitment contract.


Mathematically, this is like a Stackelberg game with Zero self moving first to choose a contract in anticipation of One self's best response recontracting. }   
...
\end{equation}
From which it is  clear that all that One self needs to react to is Zero's period zero consumption $c_0^0^$ which establishes the remaining resources to be divided between period 1 and 2.  From equation \ref{eq:budgetplus} we know that Zero wants each extra unit that it passes forward into period 1 to be, on the margin, shared equally between period 1 and period 2 consumption (i.e. the slope of the line through MF is $\frac{dc_1}{dc_0}=-\frac{1}{2}$).
...
\end{equation}
One's reaction function is depicted by the line passing through MP in the left quadrant of Figure \ref{fig:twoquad}.  As  long as $\beta< 1$,  line MP is everywhere steeper than line MF.  For any given period 0 consumption level, One wants to consume more in period 1 and less in period 2 compared to Zero's preferences.
...
As Figure \ref{fig:c1c2} makes clear, Zero self has little choice but to accommodate to the fact One will grab consumption resources that Zero self would have preferred to earmarked for period 2 consumption.  It is easy to see that for all $0<\beta < 1$ line $\Lambda^\frac{1}{\rho}>\beta^\frac{1}{\rho}$ and hence line OP is everywhere steeper  than line OF.   We can combine this  with the earlier observation that line MP
is everywhere steeper than line MF
to conclude that $c_1^P>c_1^F$ . In words, the absence of self control leads to higher period one consumption compared to a situation where self-control can be exercised. 

With a little extra algebra we can also show that c_0^P\gtreqless_0^F$ as $\rho \lesseqgtr 1$ for $0<\beta<1$ but also that c_2^P<c_2^F$.  When $\rho<1$
Zero self responds to the self-control problem by reducing savings in period 0. When  $\rho>1$ she responds by increasing savings.  This increase in period 0  savings is however never enough to lead to a net increase in period 2 consumption. Indeed the changes in period 0 savings are very small relative to the decrease in net-savings in period 1.  In other words for Zero self can do little other than to  just largely accommodate to the pattern of consumption that One self wants to impose.  

For our earlier parameterization $\beta=0.5$ and $\rho=1$ the best contract in the absence of self-control will be $C_0^P=(150, 100, 50) $ which offers considerably less consumption smoothing in later periods compared to the contract with self-control $C_0^F=(150, 75, 75) $.  If the consumer has income stream $Y_0=(100, 100, 100)$ then we can think of the consumer with self-control as sticking to a balanced repayment program to keep consumption steady in the last two periods. Compared to this the consumer without self-control postpones rolls over rather than repays the portion of the debt that they would have paid off in period one, lowering period 2 consumption. If the consumer had had an income stream $Y_0=(200, 50, 50)$ then a consumer without self-control can be viewed as raiding   savings in period that an otherwise identical consumer with self-control would have earmarked to boost period 2 consumption. Put differently, `solving'   the self control problem  leads consumers to save more/borrow less in period 1 and consume more in period 2. 

The diagrams  have focused on the special case where $\rho=1$ which implies an elasticity of inter-temporal substitution of $\sigma=\frac{1}{\rho}=1$.  This implies the consumer that period zero consumption will be the same under self-control or its absence. For $\rho\neq1$ it can be shown that $c_0^P>c_0^F$ as $\rho<1$ and or $c_0^P<c_0^F$ as $\rho>1$.  
The consumer may raise or lower period 0 consumption as part of their effort to indirectly control their latter self's choices.  However the amount by which $c_0^P$ will differ from $c_0^F$ is  very small compared to the movement in period 1 and period 2 consumption, for all parameters values in the range $0\leq \beta \leq 1$ and $0\leq \rho \leq \infty $. In practical terms this means that, in the absence of self-control, Zero can do very   little but accommodate to One's preferences in establishing the balance of consumption between period 1 and period 2. This relative powerlessness  is what  creates the demand for commitment services from a financial intermediary.

?
 

Figure  \ref{fig:renegotiation} below depicts the self-control or renegotiation problem in period 1 for the monopoly case. As we did before, assume for the sake of argument only that Zero had agreed to the full-commitment contract indicated by point $mF$ in the figure. 
At the start of period 1 this contract satisfies Zero self's optimality condition $u'(c_1^{mF})=u'(c_2^{mF})$ but from the standpoint of One self's preferences involves too little period 1 consumption as $u'(c_1^{mF})\geq \beta u'(c_2^{mF})$. Zero self's indifference curve is tangent to the bank's isoprofit line but One self's indifference curve is steeper than the budget line. The monopolist Bank can gain the most by offering to recontract to point R  along the $c_2=\beta^\frac{1}{\rho}c_1$ ray. The bank profits by pandering to One's present bias.  If such a recontracting did in fact take place the bank will have succeeded in extracting a new profits in period 1 in such a way that pushed Zero's intertemporal utility \textit{below} her reservation utility. A sophisticated customer will of course anticipate this happening and will only agree to renegotiation-proof contracts in period 0 that protect her against this type of opportunistic recontracting.
The renegotiation-proof contract must therefore lie along the $c_2=\beta^\frac{1}{\rho}c_1$ ray (as Zero understands that One would otherwise try to renegotiate to a point on this ray anyway) at a point where the ray hits Zero's autarky iso-utility surface. The monopolist pays the price for his inability to commit to not renegotiating the contract as profits can only be lowered by displacing the equilibrium contract from the optimum at $mF$. 
...
More formally, consider the renegotiation problem at the start of a subgame in period 1. A customer who has agreed to contract $C_{0}$ in period 0 enters
...
The previously analyzed point R in Figure \ref{fig:renegotiation} is the renegotiated contract
