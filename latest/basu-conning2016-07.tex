...
\subsection{Consumers and their dueling selves}
...
\subsection{Consumption smoothing with and without self-control} \label{sec:own}
...
Zero self finds herself therefore in a  strategic contracting game with  her latter period self. The contracts she may demand, and that she may be offered, and therefore the consumption smoothing she can achieve  will be quite different compared to situations where this self-control problem can assumed away or resolved at low cost.  
...
To establish a reference benchmark against which we can highlight the costly consequences of the absence of  self-control mechanisms let us start by assuming a simple situation where the consumer has access to informal borrowing or saving at constant competitive interest rate \(  r\). Suppose also that the consumer faced no self control problem: Zero self chooses a contract that costlessly commits her period-1 and period-2 selves to follow the consumption plan laid out by the contract. This is  a standard utility maximization problem subject to an   inter-temporal budget constraint
...
\end{equation}
 ...
 Along with a binding budget constraint these equations allow us to solve for the optimal  competitive `full-commitment' contract $C^{0F}$ so called because latter period selves are commited to not change it. Conceptually the equilibrium contract will be found at the tangency between the highest iso-utility surface  just touching the budget hyper-plane. A 3D graphical analysis is possible but cumbersome so draw insight from 2D contour projections instead.         
...
Equations \ref{eq:FOCc0c1} and \ref{eq:budgetplus} allow us to solve for $c_0$ and then  the remaining periods.  The term $\delta (1+r)$ enters each expression above and an increase or decrease essentially `tilts' consumption to be more generally rising or falling over time as  $\delta \gtreqless 1/(1+r)$.  As this general tilt does not alter any of the key tradeoffs of interest (unlike the degree of present-bias $\beta$ which will)  without loss of generality we shall impose the assumption that  \(\delta=\frac{1}{1+r}\) for the remainder of the analysis, to greatly unclutter the math expressions. Equations \ref{eq:FOCc0c1} to \ref{eq:budgetplus} then collapse simply to
...
These relationships and Zero's preferred contract solution be seen in the two-panel diagram of Figure \ref{fig:twoquad},  drawn for a consumer with $\beta = 0.5$ and $\rho=1$ and a present value of income \(y=300\) spread over the three periods (we are assuming \(r=0\)).\footnote{These parameter values are chosen for expositional purposes. In particular  $\rho=1$ implies that period zero consumption will  remain the same with or without self-control which slightly simplifies the graphical exposition\ (by placing points F and P along the same first period budget line) but the analysis is easily adapted to other cases.} 
...
Mathematically, this has the structure of a Stackelberg game with Zero self moving to choose a contract first in anticipation of One self's best recontracting best response. }   
...
\end{equation}
Analagously to what happens in a classic Stackelberg Cournot duopoly game our Zero self wants to choose the consumption contract  that puts her on the highest iso-utility surface along One's reaction function. Contract \(P \text{ }\)in Figure satisfies both equation \ref{eq:react1} (the line through MP) and equation \ref{eq:LambdaC0} (the line through OP). These two equations can be solved to give a closed-form solution to the optimal contract in the absence of self-control \begin{equation}
...
As can be seen in Figure \ref{fig:c1c2}, Zero self has little choice but to accommodate to the fact One self will grab a fraction of any of the consumption resources that Zero self would have liked to have made available or period 2 consumption.  It is easy to show that for all $0<\beta < 1$ line $\Lambda^\frac{1}{\rho}>\beta^\frac{1}{\rho}$ and hence line OP is everywhere steeper  than line OF.  We can combine this  with the earlier observation that line MP
is everywhere steeper than line MF
to conclude that $c_1^P>c_1^F$ so the absence of self control leads to higher period one consumption compared to a situation where self-control can be exercised. 
...
The consumer may raise or lower period 0 consumption as part of their effort to indirectly control their latter self's choices.  However the amount by which $c_0^P$ will differ from $c_0^F$ is  very small compared to the movement in period 1 and period 2 consumption, for all parameters values in the range $0\leq \beta \leq 1$ and $0\leq \rho \leq \infty $. In practical terms this means that, in the absence of self-control, Zero can do very   little but accommodate to One's preferences in establishing the balance of consumption between period 1 and period 2. This relative powerlessness  is what  creates the demand for commitment services from a financial intermediary, the topic which we turn to next.   

We have analyzed the consumer as having access to competitive financing options in the `informal economy' that allows her to save or borrow at a fixed interest rate \(r \) and then analyzed the consumer's frustrated consumption smoothing in the absence of self-control.  It may be more realistic to assume however that in the informal economy -- in the absence of  formal banking services -- the consumer may have only very limited access to saving or  borrowing, 
 If
the consumer's income is back-heavy, borrowing constraints may limit her ability
...
stream with own savings strategies but technological and/or institutional restrictions  may limit the returns to saving \(   r\) to possibly negative rates, with insecurity of storing cash at home being one obvious
explanation.  

In the analysis below it will be important to often refer to the consumer's autarky or reservation utility -- the level of utility that they could obtain by not contracting with a financial intermediary.  In these cases we will typically work by first assuming that conditions in the informal `autarky' economy are such that the consumer has access to no effective financing or own saving strategies  and  therefore can do no better than consume their autarky income stream. We later show how equilibrium contracts change when the consumer has some informal  financing or own savings options that raise their reservation utility.  
...
\section{Bank opportunism and costly commitment}

In the last section we worked under the assumption that the consumer had access to competitive `informal' finance and contrasted two polar opposite contracting scenarios: (1)\ with self-control, where Zero could enter into a multi-period commitment contract that bound her latter self to not renegotiate or refinance the contract; and (2) with no self-control, where Zero choose a contract aware that she could not prevent her latter self from turning back to the same or a different intermediary to refinance the contract. 
In the following sections we expand the analysis to monopoly and competitive contracts in situations with and without exclusive contracts. We  also now study  more realistic contracting situations where bank commitment services (i.e. their promise to agree to not recontract with the consumer's latter period selves)
must be self-enforcing in ways that reduce the contract space and impose costs.
This will lead us to an analysis of  how banks may alter ownership and governance forms to strategically reduce reduce costs and expand contracting opportunities.   

The Zero consumer has the option of contracting with one or many
risk-neutral banks, depending on the market structure which could be monopolized or competitive. As before each bank has
access to funds that can be withdrawn from other investments at an
interest rate $r$, which as before we normalize to \(r=0\).
A bank will participate and offer a contract if and only if it can earn non-negative
profits. Now however we will be specify the explicit cost to the bank of  renegotiating a contract and study the bank's  motives and ability to offer credible commitment services or not.
...
The first-order conditions  are the same as we saw in expressions \ref{eq:FOC_comp}, reproduced below with $\delta= 1/(1+r)$ :
\begin{equation}
u'\left(c_{0}\right)=\beta u'\left(c_{1}\right)=\beta u'\left(c_{2}\right)
\end{equation}which for the CRRA case become $c_1=c_{2}=\beta^\frac{1}{\rho}c_{0}$.

They require
that the discounted marginal utilities of consumption be equalized
across periods. Combined with the Zero's participation constraint (which must bind at a monopoly optimum)
yields the solution, denoted $C_{0}^{mF}$ (the superscript is for
`monopoly, full commitment') and corresponding bank profits $\Pi_{0}\left(C_{0}^{mF};Y_{0}\right)$
. Closed form solutions for the CRRA case appear as appendix equations \ref{eq:c-mf} and \ref{eq:pi-mf}, respectively. On the right panel  diagram of Figure \ref{fig:twoquad} the solution would be found at the intersection of line MF ($c_1=\beta^\frac{1}{\rho}c_{0}$) and the Zero indifference contour given by \ref{eq:PC}, and on the left panel and in Figure X below at the intersection of the line through OF ($c_1=c_2$) and the indifference contour  given by \ref{eq:PC}.
...
\subsection{Competitive Bank contracts}

Under the assumption that banks can commit to not renegotiate contracts, competition will lead a bank to offer the  full-commitment contract $C_{0}^{F}$ of expression \ref{eq:CF} as analyzed in section \ref{sec:own}. Banks get zero profits and the consumer perfectly smoothes
consumption and gets utility $U_{0}^{*}$. The equilibrium contract
is not dependent on the consumer's autarky utility.

This contract was derived  under the dual assumption that the bank can offer costlessly \emph{non-renegotiable} and \emph{exclusive} contracts. By non-renegotiable we mean that the bank's promise to not renegotiate the contract with One is credible and costlessly enforced. \y By exclusive we mean that other banks can be prevented from establishing
any new or additional contracts with the customer and her future selves
in any period. 
We shall show that in the costly commitment scenarios the firms'
strategic choice to operate as a non-profit or hybrid corporate form may be different depending on whether the contracts
...
the future). Given a particular autarky utility, the full-commitment contract solutions
above do not depend on any assumptions of sophistication or naivete. It should be noted however that in cases where the consumer self-saves in
