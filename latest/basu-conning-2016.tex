...
$C_1 =(c_{1}^{0},c_{2}^{0})$ of the contract $C_{0}$ with a lower-cost renegotiated
contract $C_{1}^{1}(C_{1})=(c_{1}^{1},c_{2}^{1})$ does not exceed renegotiation cost
$\kappa$.
Here $C_1^1$ refers to One's best response to $C_1$, the period 1 and period 2 consumption plan  laid out in the original contract $C_0$. The shape of $C_{1}^{1}(C_{1})$ will depend on whether in period 1 the market for refinances is competitive or monopolized (or somewhere in between) as analyzed below.  

 

This renegotiation-proof constraint can be stated as  

\begin{equation}
\Pi(C_{1}^{1}(C_{1}),C_{1})\leq \kappa
\end{equation}   
If we write $C_1^1=(c_1^1,c_2^1)$ and $C_{1} =(c_1^0,c_2^0)$, where the latter is the contract initially chosen by Zero and the latter is the refinance offered to/chosen by One, then the constraint is simply: 
\begin{equation*}
c_1^0+c_2^0-c_1^1+c_2^1 \leq \kappa
\end{equation}

Consider first the competitive case with zero renegotiation costs $\kappa$. Since there is no cost to renegotiation the bank 
